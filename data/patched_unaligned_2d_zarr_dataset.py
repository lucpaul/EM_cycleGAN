...
class PatchedUnaligned2dZarrDataset(BaseDataset2D):
    """
    Dataset for loading unaligned/unpaired 2D datasets from Zarr or TIFF for training.

    Requires two directories to host training images from domain A ('/path/to/data/trainA')
    and from domain B ('/path/to/data/trainB').
    Used during training of a 2D model.
    """

    def __init__(self, opt):
        """
        Initialize the PatchedUnaligned2dZarrDataset.
        ...
        logging.info("A_paths: %s", self.A_paths)
        ...
        if opt.use_zarr:
            if self.A_paths[0].endswith("tif"):
                ...
                for dataset_A in self.datasets_A:
                    if len(dataset_A.shape) == 3:
                        self.is_pre_extracted_A.append(True)
                    elif len(dataset_A.shape) == 4:
                ...
                for dataset_B in self.datasets_B:
                    if len(dataset_B.shape) == 3:
                        self.is_pre_extracted_B.append(True)
                    elif len(dataset_B.shape) == 4:
    ...
    def __getitem__(self, index):
        ...
        if self.data_is_zarr:
            ...
            if self.is_pre_extracted_A[dataset_idx_A]:
                ...
                s, h, w = patch_info_A[1:]
                if hasattr(self, "use_caching") and self.use_caching:
                    patch_A = self._get_patch_cached(self.datasets_A, dataset_idx_A, s, h, w)
                else:
                    patch_A = dataset_A[s, h : h + ph, w : w + pw]
            ...
            if self.is_pre_extracted_B[dataset_idx_B]:
                ...
                s, h, w = patch_info_B[1:]
                ph, pw = self.patch_size
                if hasattr(self, "use_caching") and self.use_caching:
                    patch_B = self._get_patch_cached(self.datasets_B, dataset_idx_B, s, h, w)
                else:
                    patch_B = dataset_B[s, h : h + ph, w : w + pw]
        else:
        ...
        patch_A = self.transform_A(patch_A)
        patch_B = self.transform_B(patch_B)
    ...
    def _get_patch(self, datasets, dataset_idx, s, h, w):
        """
        Extract a 2D patch from a full 3D volume dataset. This function is cached to improve performance.
        ...
        s, h, w : int
            Slice, height, and width indices.
    ...
    for dataset_idx, dataset in enumerate(datasets):
        if is_pre_extracted[dataset_idx]:
            ...
            for s in range(S):
                for h in h_indices:
                    for w in w_indices:
                        patch_indices.append((dataset_idx, s, h, w))
...
def save_patches_to_zarr(patch_list, output_path):
    ...
    patch_list : list of np.ndarray
        List of 2D patches as NumPy arrays.
    output_path : str
