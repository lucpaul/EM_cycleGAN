    ...
    for j, data in enumerate(dataset):
        ...
        for i in tqdm(range(0, len(input_list)), desc="Inference progress"):
            ...
            #    prediction_map = np.zeros((size_0, size_1, size_2), dtype=np.uint8)
                prediction_map = np.zeros((data['A_full_size_pad'][0], data['A_full_size_pad'][1], data['A_full_size_pad'][2]), dtype=np.uint8)
                # prediction_map = torch.zeros(size_0, size_1, size_2, dtype=torch.float32, device="cuda")
                prediction_slices = build_slices_3d(prediction_map, [patch_size, patch_size, patch_size], [patch_size, patch_size, patch_size])
