...
from . import BaseModel3D
from . import networks_3d, SSIM


class CycleGAN3DModel(BaseModel3D):
    """
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            #self.criterionCycle = SSIM.SSIM3D()
    ...
    def backward_G(self):
        ...
        # GAN loss D_A(G_A(A))
        self.loss_G_A = self.criterionGAN(self.netD_A(self.fake_B), True)# + 0.1*(1-self.criterionCycle(self.real_A,self.fake_B))
        # GAN loss D_B(G_B(B))
        self.loss_G_B = self.criterionGAN(self.netD_B(self.fake_A), True)# + 0.1*(1-self.criterionCycle(self.real_B,self.fake_A))
