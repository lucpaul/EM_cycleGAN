...
from . import networks_3d, SSIM

class CycleGANValid3dModel(BaseModel3D):
    """
    ...
    def __init__(self, opt):
        """Uses non-valid convolutions for the generator.
        opt.non_valid_conv_padding: if 'none', no padding -> valid convolution

        Parameters:
            opt (Option class)-- stores all the experiment flags; needs to be a subclass of BaseOptions
        """
        BaseModel3D.__init__(self, opt)
        #self.scaler_G = torch.cuda.amp.GradScaler()
        #self.scaler_G_B = torch.cuda.amp.GradScaler()
        ...
        if self.isTrain:
            ...
            self.criterionCycle = SSIM.SSIM3D()#CroppedLoss(SSIM.SSIM3D())
            #self.criterionCycle = CroppedLoss(torch.nn.L1Loss())
            self.criterionIdt = torch.nn.L1Loss()#CroppedLoss(torch.nn.L1Loss())
    ...
    def forward(self):
        ...
        self.fake_B_shape = self.fake_B.shape   # This will be used to scale the images in the visualizer
    ...
    def backward_G(self):
        ...
        self.loss_G_A = 0.8*self.criterionGAN(self.netD_A(self.fake_B), True) + 0.2 * (1 - self.criterionCycle(self.real_A, self.fake_B))
        # GAN loss D_B(G_B(B))
        #self.loss_G_B = self.criterionGAN(self.netD_B(self.fake_A), True)
        self.loss_G_B = 0.8*self.criterionGAN(self.netD_B(self.fake_A), True) + 0.2 * (1 - self.criterionCycle(self.real_B, self.fake_A))
